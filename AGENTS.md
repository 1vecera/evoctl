...
Run `uv run ruff check .`, `uv run ruff format --check .`, `uv run pyright`, `uv run pytest`, and `uv build` before proposing a change. Tests may use a local protocol fixture, but live tests must stay read-only unless the operator provides a reviewed recipient and exact message.
